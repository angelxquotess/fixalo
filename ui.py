...
class HudCanvas(QWidget):
    ...
    def set_paused(self, paused: bool) -> None:
        """Pause/resume the waveform repaint loop (Fase 4 CPU saving)."""
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        # ---- Fase 4: tray + floating icon state ----
        self._is_minimized_to_tray: bool = False
        self._floating_icon = None       # type: ignore[assignment]
        self._tray_manager = None        # type: ignore[assignment]
        self._cam_last_ts: float = 0.0   # for min-mode camera throttling

    def _on_floating_icon_click(self) -> None:
        """Left-click on the floating arc reactor restores the HUD."""
        self.restore_from_tray()

    def minimize_to_tray(self) -> None:
        """Hide the main HUD, show floating arc-reactor + tray icon,
        pause the waveform repaint and throttle camera preview to 1 fps."""
        if self._is_minimized_to_tray:
            return
        self._is_minimized_to_tray = True

        # CPU saver #1 — pause waveform
        try:
            self.hud.set_paused(True)
        except Exception:
            pass

        # Lazy import (pystray / PIL only needed here)
        try:
            from tray_manager import FloatingArcReactor, TrayManager
        except Exception as exc:
            self._log.append_log(f"SYS: tray unavailable ({exc}) — window only hidden")
            FloatingArcReactor = None
            TrayManager = None

        # 1) Floating draggable icon
        if FloatingArcReactor is not None:
            if self._floating_icon is None:
                def _mute_from_floating_icon():
                    # Runs on the Qt main thread (mouse event handler),
                    # so it's safe to call the real toggle directly —
                    # no QTimer marshaling needed like the tray backend.
                    self._toggle_mute()
                    return self._muted

                self._floating_icon = FloatingArcReactor(
                    on_mute_toggle=_mute_from_floating_icon
                )
                self._floating_icon.restore_requested.connect(
                    self._on_floating_icon_click
                )
                # position: top-right of the primary screen
                scr = QApplication.primaryScreen().availableGeometry()
                self._floating_icon.move(scr.right() - 110, scr.top() + 60)
            # keep icon color in sync with current mic state
            try:
                self._floating_icon.set_muted(self._muted)
            except Exception:
                pass
            self._floating_icon.start_anim()
        ...
        if TrayManager is not None:
            def _show_from_tray():
                ...
                QTimer.singleShot(0, self.restore_from_tray)

            def _exit_from_tray():
                QTimer.singleShot(0, QApplication.quit)

            def _mute_from_tray():
                # Executed in pystray's background thread → marshal to Qt.
                # We need to return the NEW mute state to the tray so the
                # icon paints itself correctly, so we compute it here.
                new_state = not self._muted
                QTimer.singleShot(0, self._sync_mute_state_with_tray)
                return new_state

            if self._tray_manager is None:
                self._tray_manager = TrayManager(
                    _show_from_tray, _exit_from_tray,
                    on_mute_toggle=_mute_from_tray,
                )
            # keep tray icon in sync with current mic state
            try:
                self._tray_manager.set_muted(self._muted)
            except Exception:
                pass
        ...
        # 3) Hide main window
        self.hide()
        self._log.append_log("SYS: Minimized to tray (say 'mostrati' to restore)")

    def restore_from_tray(self) -> None:
        """Reverse of ``minimize_to_tray`` — restore the main HUD."""
        if not self._is_minimized_to_tray:
            # still make sure window is visible
            self.showNormal()
            self.raise_()
            self.activateWindow()
            return
        ...
        # kill floating icon
        try:
            if self._floating_icon is not None:
                self._floating_icon.stop_anim()
        ...
        self.raise_()
        self.activateWindow()
        self._log.append_log("SYS: Restored from tray")
    ...
    def _on_cam_frame(self, data: bytes) -> None:
        # Fase 4: throttle to ~1 fps when minimized to tray → CPU saver
        if self._is_minimized_to_tray:
            _now = time.monotonic()
            if _now - self._cam_last_ts < 1.0:
                return
            self._cam_last_ts = _now
        px = QPixmap()
    ...
    def _build_header(self) -> QWidget:
        ...
        # Minimize-to-tray button ("–") — Fase 4
        lay.addSpacing(10)
        self._min_tray_btn = QPushButton("–")
        self._min_tray_btn.setFixedSize(26, 26)
        self._min_tray_btn.setFont(QFont("Courier New", 14, QFont.Weight.Bold))
        self._min_tray_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._min_tray_btn.setToolTip("Minimize to tray (voice: 'minimizzati')")
        self._min_tray_btn.setStyleSheet(f"""
            QPushButton {{
                background: transparent; color: {C.TEXT_DIM};
                border: 1px solid {C.BORDER}; border-radius: 4px;
                padding-bottom: 4px;
            ...
            QPushButton:hover {{ color: {C.PRI}; border-color: {C.PRI_DIM}; }}
        """)
        self._min_tray_btn.clicked.connect(self.minimize_to_tray)
        lay.addWidget(self._min_tray_btn)
        return w
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("By NotVortex", C.PRI_DIM))
        return w
    ...
    def _toggle_mute(self):
        ...
        self._style_mute_btn()
        # Sync tray icon color (red when muted) if the tray is up.
        try:
            if self._tray_manager is not None:
                self._tray_manager.set_muted(self._muted)
        except Exception:
            pass
        # Sync floating arc-reactor icon color (red when muted) too.
        try:
            if self._floating_icon is not None:
                self._floating_icon.set_muted(self._muted)
        except Exception:
    ...
    def _sync_mute_state_with_tray(self):
        """Called from the Qt main thread when the tray toggled mute.
        The tray already flipped its internal state optimistically; we
        just need to mirror that into the real app-wide mute state."""
        self._toggle_mute()
...
class JarvisUI:
    ...
    def minimize_to_tray(self) -> None:
        """Thread-safe: hide HUD into system tray + floating icon (Fase 4)."""
        QTimer.singleShot(0, self._win.minimize_to_tray)

    def restore_from_tray(self) -> None:
        """Thread-safe: restore HUD from tray."""
        QTimer.singleShot(0, self._win.restore_from_tray)

    @property
    def is_minimized_to_tray(self) -> bool:
        return getattr(self._win, "_is_minimized_to_tray", False)
